...
/*   Updated: 2024/07/30 10:33:45 by cjuarez          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				char ***arg_list, char **ls)
{
	...
	if (ft_strcmp(argv[0], "here_doc") && !check_ls(ls, "ls")
		&& !check_ls(ls, "wc"))
...
void	pipex(char **argv, char **envp)
{
	...
	if (ft_strcmp(argv[0], "here_doc"))
		c = get_next_line(0, argv[1], -2);
	else if (!c && !ft_strcmp(argv[1], "/dev/stdin"))
		c = file_read(argv[1]);
	arg_list = fill_arg_list(argv + 2, i);
	if (!file_check_w(argv[2 + i]))
		arg_list[i - 1][0] = fill_acc(arg_list[i - 1][0]);
	cmnd_list = fill_cmnd_list(arg_list, envp, i, c);
	if (!c && ft_strcmp(argv[1], "/dev/stdin")
		&& !ft_strcmp(*(cmnd_list), "nil"))
		c = special_dev(cmnd_list, arg_list, 0, envp);
	continue_pipex(argv, c, arg_list, cmnd_list);
}
